...
class SearchBusinessPage extends React.Component {
  ...
  componentDidMount() {
    if (this.props.location.search.slice(1).length > 0) {
      this.props.searchBusinesses(
        `dropdown=(true)&price=()&noise=()&delivery=()&name=${this.props.location.search.slice(
          1
        )}`
      );
    } else {
  ...
  componentWillUpdate(nextProps, nextState) {
    if (this.state !== nextState) {
      ...
      this.props.searchBusinesses(
        `dropdown=(true)&price=()&noise=()&delivery=()&name=${nextProps.location.search.slice(
          1
        )}`
      );
    }
    ...
    return (
      <ul className="top-wrapper3-sec-1">
        {[1, 2, 3, 4].map((val, idx) => {
          return (
            <li
              className={this.renderToggle(type, val)}
    ...
    return (
      <div>
            ...
            <div className="top-category-results">
              <div className="top-category-results1">
                <div className="top-res-name">
                  <div className="top-results-home">{bestResults}</div>
                </div>
        ...
        <div className="main-business-container">
          <div className="business-container">{searchedBusinesses}</div>
          <div className="sear-ad-cont">
            <BusinessMap
              id={"map-container"}
              businesses={this.props.searchedBusinesses}
            />
