...
class SearchForm extends React.Component {
  ...
  handleSubmit(e) {
    e.preventDefault();
    this.props.requestDropdownBusinesses(
      `dropdown=(true)&price=()&noise=()&delivery=()&name=${this.state.name}`
    );
    this.setState({ name: "" });
  ...
  toggleIcon() {
    if (
      ...
      this.props.className === "sear-container"
    ) {
      return <div />;
        ...
        <div>
          <div className="logout-nav">
            <img
              ...
              className="drop-down-pic"
              src="https://s3.amazonaws.com/betterhelp-dev/dropdown.png"
            />
            <div onClick={this.toggleClass} className="caret-drop-down">
              <i className="fas fa-caret-down" />
            </div>
          </div>
          <i className={hiddenCar} />
          <div className={toggle}>
            <div className="dropped-down-first">
              <img
                className="dropped-down-pic"
                src="https://s3.amazonaws.com/betterhelp-dev/dropdown.png"
              />
              <div className="dropped-down-info-wrapper">
                <div className="dropped-down-name">
                  <Link
                    ...
                    to={`/user_prof/${this.props.currentUser.id}`}
                  >
                ...
                <div className="dropped-down-location">Manhattan, NY</div>
              </div>
            </div>
            ...
            <button
              className="dropped-down-logout-btn"
      ...
      return (
        <div className="login-signup-nav">
          <Link className="log-in" to="/login">
            {" "}
            <div className="login-sear-nav">Log In</div>
          </Link>
          <Link className="log-in" to="/signup">
            {" "}
            <div className="signup-sear-nav">Sign Up</div>
          </Link>
        </div>
      );
    }
  ...
  render() {
    let searchedBusinesses1 = this.props.searchedBusinesses.map(
      (business, idx) => (
        <SearchDropDown
          key={business.id}
          business={business}
    ...
    return (
      <div className={this.props.className}>
        <form
          className={`search-form` + `-${this.props.className}`}
          onSubmit={this.handleSubmit}
        >
          <div className={"find" + `-${this.props.className}`}>Find</div>
          <DebounceInput
          ...
          <div className={"near" + `-${this.props.className}`}>Near</div>
          <input
            placeholder="Current Location"
            onChange={this.update("searchLocation")}
            className={"location" + `-${this.props.className}`}
            onKeyDown={this.keydown}
            type="search"
            value={this.state.searchLocation}
          />

          <button className={"search-pic" + `-${this.props.className}`}>
        ...
        <div
          className={
            "drop-down" + `-${this.props.className} ${this.state.dropdown}`
          }
        >
          {searchedBusinesses1}
        </div>
